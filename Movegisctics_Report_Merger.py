...
def merge_files(f_ai, f_jo, f_op):
    ...
    # ── Merge 2: + OP on Customer ID ─────────────────────────────
    m2 = pd.merge(m1, op_slim, on='Customer ID', how='left')
    # Coalesce Estimated from OP where missing
...
# ── Header ────────────────────────────────────────────────────────────────────
st.markdown("""
<div class="hero-title">📦 Movegistics Reports Builder<span class="version-badge">v1.1</span></div>
...
with tab1:
    ...
    with right:
        st.markdown('<div class="section-header">⚙️ Merge Logic</div>', unsafe_allow_html=True)
        st.info(
            "**Step 1:** JobOverview ← ActualIncome on **WO Id**\n\n"
            "**Step 2:** + Opportunities on **Customer ID**\n\n"
            "**Unified columns:**\n"
            "- Customer ID · Customer Name\n"
            "- Estimated · Move Date · Date Booked\n\n"
            "**Removed:** Expected Close Date · duplicate Location Type · # index"
        )
        st.markdown("<br>", unsafe_allow_html=True)

        if st.button("🔗 Merge Files", disabled=not (f1 and f2 and f3), use_container_width=True):
            with st.spinner("Merging your CRM files..."):
                try:
                    df = merge_files(f1, f2, f3)
                    st.session_state['df'] = df
                    st.session_state['filtered_df'] = df
                    st.success(f"✅ Done! **{df.shape[0]:,} rows** × **{df.shape[1]} columns**")
    ...
    if 'df' not in st.session_state:
        ...
        with s1:
            if 'Grand Total' in fdf.columns:
                gt = pd.to_numeric(
                    fdf['Grand Total'].astype(str).str.replace(r'[$,]', '', regex=True),
                    errors='coerce'
                )
...
with tab3:
    if 'df' not in st.session_state:
        ...
        st.markdown('<div class="section-header">💾 Download Merged Data</div>', unsafe_allow_html=True)
        e1, e2 = st.columns(2)
